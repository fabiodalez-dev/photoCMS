...
class LensesController extends BaseController
{
    ...
    public function create(Request $r, Response $res): Response {return $this->view->render($res,'admin/lenses/create.twig',['csrf'=>$_SESSION['csrf']??'']);}
    public function store(Request $r, Response $res): Response{
        ...
        if($brand===''||$model===''){ $_SESSION['flash'][]=['type'=>'danger','message'=>'Brand and Model are required']; return $res->withHeader('Location',$this->basePath . '/admin/lenses/create')->withStatus(302);}        
        try{ $this->db->pdo()->prepare('INSERT INTO lenses(brand, model, focal_min, focal_max, aperture_min) VALUES(?,?,?,?,?)')->execute([$brand,$model,$fmin,$fmax,$amin]); $_SESSION['flash'][]=['type'=>'success','message'=>'Lens created']; }
        catch(\Throwable $e){ $_SESSION['flash'][]=['type'=>'danger','message'=>'Error: '.$e->getMessage()]; return $res->withHeader('Location',$this->basePath . '/admin/lenses/create')->withStatus(302);}        
    ...
    public function update(Request $r, Response $res, array $args): Response{
        ...
        $fmin=$d['focal_min']!==''?(float)$d['focal_min']:null; $fmax=$d['focal_max']!==''?(float)$d['focal_max']:null; $amin=$d['aperture_min']!==''?(float)$d['aperture_min']:null;
        if($brand===''||$model===''){ $_SESSION['flash'][]=['type'=>'danger','message'=>'Brand and Model are required']; return $res->withHeader('Location',$this->basePath . '/admin/lenses/'.$id.'/edit')->withStatus(302);}        
        try{ $this->db->pdo()->prepare('UPDATE lenses SET brand=?, model=?, focal_min=?, focal_max=?, aperture_min=? WHERE id=?')->execute([$brand,$model,$fmin,$fmax,$amin,$id]); $_SESSION['flash'][]=['type'=>'success','message'=>'Lens updated']; }
        catch(\Throwable $e){ $_SESSION['flash'][]=['type'=>'danger','message'=>'Error: '.$e->getMessage()]; }
        return $res->withHeader('Location',$this->basePath . '/admin/lenses')->withStatus(302);
    }
    public function delete(Request $r, Response $res, array $args): Response{ $id=(int)($args['id']??0); try{$this->db->pdo()->prepare('DELETE FROM lenses WHERE id=:id')->execute([':id'=>$id]); $_SESSION['flash'][]=['type'=>'success','message'=>'Lens deleted'];}catch(\Throwable $e){$_SESSION['flash'][]=['type'=>'danger','message'=>'Error: '.$e->getMessage()];} return $res->withHeader('Location',$this->basePath . '/admin/lenses')->withStatus(302);}    
}
