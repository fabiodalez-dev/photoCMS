...
class FilmsController extends BaseController
{
    ...
    public function store(Request $r, Response $res): Response{
        ...
        if($brand===''||$name===''){ $_SESSION['flash'][]=['type'=>'danger','message'=>'Brand and Name are required']; return $res->withHeader('Location',$this->basePath . '/admin/films/create')->withStatus(302);}        
        try{ $this->db->pdo()->prepare('INSERT INTO films(brand, name, iso, format, type) VALUES(?,?,?,?,?)')->execute([$brand,$name,$iso,$format,$type]); $_SESSION['flash'][]=['type'=>'success','message'=>'Film created']; }
        catch(\Throwable $e){ $_SESSION['flash'][]=['type'=>'danger','message'=>'Error: '.$e->getMessage()]; return $res->withHeader('Location',$this->basePath . '/admin/films/create')->withStatus(302);}        
    ...
    public function update(Request $r, Response $res, array $args): Response{
        ...
        $iso=$d['iso']!==''?(int)$d['iso']:null; $format=(string)($d['format']??'35mm'); $type=(string)($d['type']??'bw');
        if($brand===''||$name===''){ $_SESSION['flash'][]=['type'=>'danger','message'=>'Brand and Name are required']; return $res->withHeader('Location',$this->basePath . '/admin/films/'.$id.'/edit')->withStatus(302);}        
        try{ $this->db->pdo()->prepare('UPDATE films SET brand=?, name=?, iso=?, format=?, type=? WHERE id=?')->execute([$brand,$name,$iso,$format,$type,$id]); $_SESSION['flash'][]=['type'=>'success','message'=>'Film updated']; }
        catch(\Throwable $e){ $_SESSION['flash'][]=['type'=>'danger','message'=>'Error: '.$e->getMessage()]; }
        return $res->withHeader('Location',$this->basePath . '/admin/films')->withStatus(302);
    }
    public function delete(Request $r, Response $res, array $args): Response{ $id=(int)($args['id']??0); try{$this->db->pdo()->prepare('DELETE FROM films WHERE id=:id')->execute([':id'=>$id]); $_SESSION['flash'][]=['type'=>'success','message'=>'Film deleted'];}catch(\Throwable $e){$_SESSION['flash'][]=['type'=>'danger','message'=>'Error: '.$e->getMessage()];} return $res->withHeader('Location',$this->basePath . '/admin/films')->withStatus(302);}    
}
