...
class FilmsController
{
    ...
    public function store(Request $r, Response $res): Response{
        ...
        if($brand===''||$name===''){ $_SESSION['flash'][]=['type'=>'danger','message'=>'Brand e Nome obbligatori']; return $res->withHeader('Location','/admin/films/create')->withStatus(302);}        
        ...
        catch(\Throwable $e){ $_SESSION['flash'][]=['type'=>'danger','message'=>'Errore: '.$e->getMessage()]; return $res->withHeader('Location','/admin/films/create')->withStatus(302);}        
        return $res->withHeader('Location','/admin/films')->withStatus(302);
    }
    ...
    public function update(Request $r, Response $res, array $args): Response{
        ...
        $iso=$d['iso']!==''?(int)$d['iso']:null; $format=(string)($d['format']??'35mm'); $type=(string)($d['type']??'bw');
        if($brand===''||$name===''){ $_SESSION['flash'][]=['type'=>'danger','message'=>'Brand e Nome obbligatori']; return $res->withHeader('Location','/admin/films/'.$id.'/edit')->withStatus(302);}        
