...
class SilktideCookieBanner {
  ...
  handleCookieChoice(accepted) {
    ...
    const consentEvent = new CustomEvent('silktideConsentChanged', {
      detail: {
        action: accepted ? 'accept' : 'reject',
        acceptedCookies: this.getAcceptedCookies()
      }
    });
  ...
  handleSelectedCookies() {
    // Save the current checkbox states from the modal
    this.setInitialCookieChoiceMade();
    ...
    // Get all checkboxes from the modal
    const preferencesSection = this.modal.querySelector('#cookie-preferences');
    const checkboxes = preferencesSection.querySelectorAll('input[type="checkbox"]');
    ...
    this.config.cookieTypes.forEach((type) => {
      ...
      if (type.required === true) {
        ...
        const checkbox = Array.from(checkboxes).find(cb => cb.id === `cookies-${type.id}`);
    ...
    // Update checkboxes in the modal with the values from localStorage
    this.updateCheckboxState();
  }
  ...
  setupEventListeners() {
    ...
    if (this.modal) {
      ...
      const saveSelectedButton = this.modal.querySelector('.preferences-save-selected');
      ...
      saveSelectedButton?.addEventListener('click', () => this.handleSelectedCookies());
...
(function () {
  ...
  function getCategoryConsent(categoryId) {
    if (!cookieBanner) {
      return false;
    }
  ...
  function openPreferences() {
    if (cookieBanner && cookieBanner.toggleModal) {
      cookieBanner.toggleModal(true);
      if (cookieBanner.hideCookieIcon) {
  ...
  window.silktideCookieBannerManager.getCategoryConsent = getCategoryConsent;
  window.silktideCookieBannerManager.openPreferences = openPreferences;

  // Alias for backwards compatibility
  window.CookieControl = {
    getCategoryConsent: getCategoryConsent,
